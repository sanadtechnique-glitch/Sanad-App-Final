...
function StarRow({ rating }: { rating?: number | null }) {
  ...
  return (
    <span className="flex items-center gap-0.5">
      {[1,2,3,4,5].map(i => (
        <Star key={i} size={11} className={i <= r ? "text-[#66BB6A] fill-[#66BB6A]" : "text-[#004D40]/15 fill-white/15"} />
      ))}
      {rating != null && <span className="ml-1.5 text-xs text-[#004D40]/40 font-mono tabular-nums">{rating.toFixed(1)}</span>}
    </span>
  );
}
    ...
    <Layout>
      <div className="pt-6 px-4 sm:px-6 lg:px-8 pb-28" dir={isRTL ? "rtl" : "ltr"}>
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-3xl font-black text-[#004D40] leading-tight">
              {t("مقدمو الخدمات", "Prestataires")}
            </h1>
            <p className="text-[#004D40]/30 text-sm mt-0.5 font-medium">
              ...
              <button key={cat.id} onClick={() => setActive(cat.id)}
                className={cn(
                  ...
                  isAct
                    ...
                    : "bg-[#004D40]/5 text-[#004D40]/50 border-[#004D40]/10 hover:text-[#004D40] hover:border-[#004D40]/25"
                )}>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {[1,2,3,4,5,6].map(i => (
              <div key={i} className="glass-panel rounded-3xl h-56 animate-pulse border border-[#004D40]/5"
          ...
          <div className="glass-panel rounded-3xl p-14 text-center flex flex-col items-center border border-red-500/20 mt-4">
            ...
            <h3 className="text-xl font-black text-[#004D40] mb-2">{t("حدث خطأ", "Erreur de chargement")}</h3>
            <p className="text-[#004D40]/30 text-sm">{t("حاول مرة أخرى", "Veuillez réessayer")}</p>
          </div>
        ) : suppliers.length === 0 ? (
          <div className="glass-panel rounded-3xl p-14 text-center flex flex-col items-center border border-[#004D40]/5 mt-4">
            ...
            <h3 className="text-xl font-black text-[#004D40] mb-2">{t("لا توجد نتائج", "Aucun résultat")}</h3>
            <p className="text-[#004D40]/30 text-sm">{t("جرّب فئة أخرى", "Essayez une autre catégorie")}</p>
          </div>
        ) : (
                    ...
                    <Link href={targetHref}>
                      <div className={cn(
                        ...
                        avail
                          ...
                          : "border-[#004D40]/5 cursor-not-allowed opacity-55"
                        ...
                        <div className="flex justify-between items-start mb-4">
                          <div className={cn("rounded-2xl flex items-center justify-center w-12 h-12 bg-gradient-to-br border border-[#004D40]/8", c.gradient)}>
                        ...
                        {/* Name */}
                        <h3 className="text-lg font-black text-[#004D40] mb-0.5 leading-snug group-hover:text-[#66BB6A] transition-colors duration-300">
                          {lang === "ar" ? s.nameAr : (s.name || s.nameAr)}
                        </h3>
                        {lang === "fr" && s.name && (
                          <p className="text-xs text-[#004D40]/20 mb-1.5 font-medium">{s.nameAr}</p>
                        )}

                        {/* Description */}
                        <p className="text-sm text-[#004D40]/40 line-clamp-2 mb-3 flex-1 leading-relaxed">
                          {lang === "ar" ? s.descriptionAr : (s.description || s.descriptionAr)}
                        </p>

                        {/* Address */}
                        <div className="flex items-center gap-1.5 text-xs text-[#004D40]/25 mb-3">
                        ...
                        {/* CTA row */}
                        {avail && (
                          <div className="mt-4 pt-3.5 border-t border-[#004D40]/5 flex items-center justify-between">
