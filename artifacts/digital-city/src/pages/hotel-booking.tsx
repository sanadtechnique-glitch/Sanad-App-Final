    ...
    return (
      <Layout>
        <div className="flex justify-center items-center min-h-[60vh]">
          <div className="w-8 h-8 border-[3px] border-[#66BB6A] border-t-transparent rounded-full animate-spin" />
        </div>
      </Layout>
    );
  }
    ...
    return (
      <Layout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] p-6 text-center">
          <Hotel size={48} className="text-[#66BB6A]/10 mb-4" />
          <p className="text-[#66BB6A]/30 font-bold">{t("الفندق غير موجود", "Hôtel introuvable")}</p>
        </div>
      </Layout>
    );
  }
    ...
    <Layout>
      <div className={cn("max-w-lg mx-auto px-4 py-6", isRTL ? "text-right" : "text-left")}>
        ...
        <div className={`mb-6 ${isRTL ? "text-right" : "text-left"}`}>
          <h1 className="text-2xl font-black text-[#66BB6A] mb-1">{t("حجز فندق", "Réservation Hôtel")}</h1>
          <p className="text-[#66BB6A]/40 text-sm">{t("أدخل تفاصيل حجزك", "Saisissez les détails de votre réservation")}</p>
        </div>
        ...
        <AnimatePresence mode="wait">
          {success ? (
            ...
            <motion.div key="success"
              ...
              className="rounded-[20px] border border-[#66BB6A]/30 p-8 text-center"
              ...
              <h2 className="text-2xl font-black text-[#66BB6A] mb-2">{t("تم إرسال الحجز! ✓", "Réservation envoyée! ✓")}</h2>
              {bookingId && (
                <p className="text-[#66BB6A]/30 text-sm mb-1">
                  {t("رقم الحجز", "N° de réservation")}: <span className="font-mono text-[#66BB6A] font-bold">#{bookingId.toString().padStart(4, "0")}</span>
                </p>
              )}
              <p className="text-[#66BB6A]/50 text-sm mb-6 leading-relaxed">
            ...
            <motion.div key="form" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
              {/* Hotel card */}
              <div className="rounded-[15px] p-4 mb-5 border border-[#66BB6A]/25 flex items-center gap-4"
                style={{ background: "#FFFFFF" }}>
                <div className="w-12 h-12 rounded-xl bg-[#66BB6A]/10 border border-[#66BB6A]/20 flex items-center justify-center flex-shrink-0">
                  <Hotel size={20} className="text-[#66BB6A]" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-black text-[#66BB6A] truncate">{lang === "ar" ? hotel.nameAr : (hotel.name || hotel.nameAr)}</p>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <MapPin size={10} className="text-[#66BB6A]/30" />
                    <p className="text-xs text-[#66BB6A]/30 truncate">{hotel.address}</p>
                  </div>
                  {hotel.rating && (
                    <div className="flex items-center gap-0.5 mt-1">
                      {[1,2,3,4,5].map(i => <Star key={i} size={10} className={i <= Math.round(hotel.rating!) ? "text-[#66BB6A] fill-[#66BB6A]" : "text-[#66BB6A]/15"} />)}
                      <span className="text-xs text-[#66BB6A]/30 ml-1">{hotel.rating.toFixed(1)}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input {...register("customerName")}
                    ...
                    className={cn("w-full bg-[#FFFDE7] border rounded-xl px-4 py-3.5 text-sm text-[#66BB6A] placeholder:text-[#66BB6A]/30",
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <Phone size={14} className={cn("absolute top-1/2 -translate-y-1/2 text-[#66BB6A]/25", isRTL ? "right-3.5" : "left-3.5")} />
                    <input {...register("customerPhone")} type="tel"
                      placeholder="+216 __ ___ ___"
                      className={cn("w-full bg-[#FFFDE7] border rounded-xl py-3.5 text-sm text-[#66BB6A] placeholder:text-[#66BB6A]/30",
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <Calendar size={13} className={cn("absolute top-1/2 -translate-y-1/2 text-[#66BB6A]/25", isRTL ? "right-3" : "left-3")} />
                      <input {...register("checkIn")} type="date" min={today}
                        className={cn("w-full bg-[#FFFDE7] border rounded-xl py-3.5 text-sm text-[#66BB6A]",
                          isRTL ? "pr-9 pl-3" : "pl-9 pr-3",
                          errors.checkIn ? "border-red-500" : "border-[#66BB6A]/40",
                          "[color-scheme:dark]")} />
                    </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input {...register("checkOut")} type="date" min={today}
                        className={cn("w-full bg-[#FFFDE7] border rounded-xl py-3.5 text-sm text-[#66BB6A]",
                          isRTL ? "pr-9 pl-3" : "pl-9 pr-3",
                          errors.checkOut ? "border-red-500" : "border-[#66BB6A]/40",
                          "[color-scheme:dark]")} />
                    </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <Users size={14} className={cn("absolute top-1/2 -translate-y-1/2 text-[#66BB6A]/25", isRTL ? "right-3.5" : "left-3.5")} />
                    <input {...register("guests")} type="number" min="1" max="20"
                      className={cn("w-full bg-[#FFFDE7] border rounded-xl py-3.5 text-sm text-[#66BB6A]",
                        isRTL ? "pr-10 pl-4" : "pl-10 pr-4",
                        errors.guests ? "border-red-500" : "border-[#66BB6A]/40")} />
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea {...register("notes")} rows={3}
                    ...
                    className="w-full bg-[#FFFDE7] border border-[#66BB6A]/40 rounded-xl px-4 py-3.5 text-sm text-[#66BB6A] placeholder:text-[#66BB6A]/25 resize-none" />
                </div>

                {/* Privacy note */}
                <div className="flex items-center gap-2 p-3 rounded-xl border border-[#66BB6A]/5" style={{ background: "rgba(255,255,255,0.02)" }}>
                  <span className="w-2 h-2 rounded-full bg-emerald-400 flex-shrink-0" />
                  <p className="text-xs text-[#66BB6A]/30">
