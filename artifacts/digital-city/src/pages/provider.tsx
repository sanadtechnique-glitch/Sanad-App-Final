      ...
      <div className="min-h-screen flex items-center justify-center p-4" style={{ background: "#FF8C00" }} dir={isRTL ? "rtl" : "ltr"}>
        <div className="w-full max-w-md">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
            ...
            <h1 className="text-3xl font-black text-[#66BB6A] mb-2">{t("لوحة المزود", "Tableau Fournisseur")}</h1>
              ...
              <motion.button key={p.id}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-right">
                    <p className="font-black text-[#66BB6A] group-hover:text-[#66BB6A] transition-colors">{p.nameAr}</p>
                    <p className="text-xs text-[#66BB6A]/30">{p.isAvailable ? t("مفتوح", "Ouvert") : t("مغلق", "Fermé")}</p>
                  </div>
                </div>
      ...
      <AnimatePresence>
        {hasNewOrder && (
          <motion.div initial={{ opacity: 0, y: -60 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -60 }}
            ...
            <span className="text-[#66BB6A] font-black text-sm">{t("🔔 طلب جديد وصل!", "🔔 Nouvelle commande!")}</span>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {photoModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.img initial={{ scale: 0.9 }} animate={{ scale: 1 }} src={photoModal}
              className="max-w-sm w-full rounded-2xl border border-[#66BB6A]/10" alt="prescription" />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="max-w-2xl mx-auto space-y-4">
            ...
            <div>
              ...
              {selected.rating && (
                <div className="flex items-center gap-0.5">
                  {[1,2,3,4,5].map(i => <Star key={i} size={11} className={i <= Math.round(selected.rating!) ? "text-[#66BB6A] fill-[#66BB6A]" : "text-[#66BB6A]/20"} />)}
                  <span className="text-xs text-[#66BB6A]/40 ml-1">{selected.rating.toFixed(1)}</span>
                </div>
              )}
            </div>
          ...
          {(["pending", "all"] as const).map(tb => (
            <button key={tb} onClick={() => setTab(tb)}
              className={cn("flex-1 py-2.5 rounded-lg font-black text-sm transition-all flex items-center justify-center gap-2",
                tab === tb ? "bg-[#66BB6A] text-black" : "text-[#66BB6A]/40 hover:text-[#66BB6A]")}>
              {tb === "pending"
                ? <>{t("جديد", "Nouvelles")} {pendingOrders.length > 0 && <span className={cn("px-1.5 py-0.5 rounded-full text-xs", tab === tb ? "bg-[#FF8C00]/20 text-black" : "bg-amber-400/20 text-amber-400")}>{pendingOrders.length}</span>}</>
                : t("كل الطلبات", "Toutes")
              }
            </button>
          ))}
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <Package size={40} className="text-[#66BB6A]/10 mx-auto mb-3" />
            <p className="text-[#66BB6A]/25 font-bold">
                    ...
                    <div className="p-4">
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <div className="flex-1 min-w-0">
                          ...
                          {order.notes && (
                            <p className="text-xs text-[#66BB6A]/30 mt-2 p-2 rounded-lg border border-[#66BB6A]/5" style={{ background: "#D4A800" }}>{order.notes}</p>
                          )}
                        </div>
