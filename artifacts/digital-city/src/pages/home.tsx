...
// ─────────────────────────────────────────────────────────────────────────────
// PARTNERS STRIP — circular logos, no frame, horizontal scroll
// ─────────────────────────────────────────────────────────────────────────────
interface PartnerItem { id: number; name: string; imageUrl: string; }

function PartnersStrip({ lang, t }: { lang: string; t: (ar: string, fr: string) => string }) {
  const [partners, setPartners] = useState<PartnerItem[]>([]);

  useEffect(() => {
    get<PartnerItem[]>("/partners").then(setPartners).catch(() => {});
  }, []);
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex items-center gap-2 px-4 mb-3" dir="rtl">
        <span className="w-1.5 h-5 rounded-full bg-[#FFA500] block flex-shrink-0" />
        <h2 className="text-base font-black text-[#1A4D1F]">{t("شركاؤنا", "Partenaires")}</h2>
      </div>

      {/* Horizontal scroll strip */}
      <div
        className="flex gap-5 px-4 overflow-x-auto no-scrollbar"
        dir="rtl"
        style={{ WebkitOverflowScrolling: "touch" }}
      >
        {partners.map(p => (
          <div key={p.id} className="flex flex-col items-center gap-1.5 flex-shrink-0">
            {/* Circular logo — no border, no frame */}
            <div
            ...
            <span style={{
              ...
              color: "#1A4D1F", textAlign: "center",
              width: 72, lineHeight: 1.2,
              fontFamily: "'Cairo','Tajawal',sans-serif",
